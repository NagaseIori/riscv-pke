...
char elf_debug_line[DEBUG_CACHE_SIZE];
int elf_debug_line_length;
elf_ctx cur_elf_ctx;
...
void elf_util_read_line(char **src, char *dst)
{
    do
    {
        *(dst++) = **src;
...
void elf_util_append_string(char *dst, char *src) {
    while(*dst) dst ++;
    elf_util_read_string(src, dst);
}

char * elf_util_read_file_line(char *file, int line) {
    static char code[CODE_CACHE_SIZE], *pcode;
    static char rcode[CODE_CACHE_SIZE];
    pcode = code;
    spike_file_t *f = spike_file_open(file, O_RDONLY, 0);

    spike_file_read(f, code, CODE_CACHE_SIZE);
    for(int i=0; i<line; i++) {
        elf_util_read_line(&pcode, rcode);
    }
    return rcode;
}
...
void elf_debug_catch(uint64 epc) {
    ...
    for(int i=0; i<p->line_ind; i++) if(epc == p->line[i].addr) {
        dir[0] = 0;
        elf_util_append_string(dir, p->dir[p->file[p->line[i].file].dir]);
        elf_util_append_string(dir, "/");
        elf_util_append_string(dir, p->file[p->line[i].file].file);
        sprint("Runtime error at %s:%d\n%s", dir, p->line[i].line, elf_util_read_file_line(dir, p->line[i].line));
        break;
    }
}

static elf_sect_header elf_read_segment(elf_ctx *ctx, char *name)
{
    ...
    elf_sect_header strtb_hd;
    elf_fpread(ctx, &strtb_hd, sizeof(strtb_hd), shoff + shstrndx * shentsize);
    char strtb_cac[strtb_hd.size];
    elf_fpread(ctx, strtb_cac, strtb_hd.size, strtb_hd.offset);

    for (int i = 0; i < shnum; i++)
    {
        elf_sect_header cur;
        elf_fpread(ctx, &cur, sizeof(cur), shoff + i * shentsize);
        char rname[256];
        elf_util_read_string(strtb_cac + cur.name, rname);
    ...
    panic("SEGMENT %s NOT FOUND", name);
}
...
void load_bincode_from_host_elf(process *p)
{
    ...
    // load debug segment to cache
    elf_sect_header _debug_line = elf_read_segment(&elfloader, ".debug_line");
    elf_debug_line_length = _debug_line.size;
    elf_fpread(&elfloader, elf_debug_line, elf_debug_line_length, _debug_line.offset);

    make_addr_line(&elfloader, elf_debug_line, elf_debug_line_length);
